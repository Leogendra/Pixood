...
const tagCategoriesReducer = (state: TagCategoriesState, action: TagCategoriesAction): TagCategoriesState => {
  switch (action.type) {
    ...
    case 'MIGRATE_FROM_OLD_SYSTEM':
      // Cette action sera implémentée pour migrer depuis l'ancien système
      return state;
...
// Convertir les émotions en tags categorisés - Version simplifiée avec 10 émotions principales
const convertEmotionsToTags = (emotionsCategory: TagCategory): CategorizedTag[] => {
  const now = new Date().toISOString();
  
  // Sélection de 10 émotions principales représentatives
  const mainEmotions = [
    'happy',      // Heureux
    'excited',    // Excité  
    'energized',  // Énergique
...
const getInitialState = (): TagCategoriesState => {
  ...
  return {
    ...
    tags: [
      ...
      // Quelques tags d'activité par défaut
      {
        id: uuidv4(),
...
export function TagCategoriesProvider({ children }: { children: React.ReactNode }) {
  ...
  const deleteTag = useCallback(async (id: string) => {
    ...
    // Supprimer le tag des logs existants
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (stored !== null) {
        ...
        // Premier lancement, initialiser avec les données par défaut
